...
const TreeMap = ({ writtenContents, poeticForms, musicalForms, danceForms, width, height }) => {
  ...
  function renderTreemap() {
      ...
      for (const formName in root[i]) {
        var internalObj = {
          ...
          artworks: artworks.filter(
            (artwork) => artwork.category === rootNames[i] && artwork.formName === formName,
          ),
        };
    ...
    block
      ...
      .on('mouseover', (artform) => {
        // mouseover effect for interaction with tiles
        tooltip.transition().style('visibility', 'visible');
        let data = artform['target']['__data__']['data'];
        let value = data['value'];
        tooltip.html(value + ' works of art.' + '<hr />' + data['name']);
