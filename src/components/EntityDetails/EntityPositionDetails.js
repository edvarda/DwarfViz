...
const EntityPositionDetails = ({ entity }) => {
    ...
    const getEntries = (position) => {
        const hfs_in_position = entity.entity_position_assignment.filter((ass) => ass.position_id == position.local_id);
        //console.log(position, hfs_in_position);
        if (hfs_in_position.length == 0) {
            ...
            const group = hfs_in_position.map((hf_in_pos) => {
                return (
                    <li>
                      <div className='propName'>{_.startCase(position.name)}:</div>
                      <div className='value'>{hf_in_pos.hf_id ? <HfLink id={hf_in_pos.hf_id}/> : 'Unknown individual'}</div>
                    </li>
                );
    ...
    return (
        <div className='detailsView'>
            ...
            <ul>
                {entity.entity_position.map(getEntries)}
            </ul>
        </div>
    );
};
